...
In your IDE, save the “workload” template that should now be completely filled out, and double-click the “workload-master.template.yaml” file. Paste the contents of the template into the file and save it. After you’ve done that, we’ll take a look at the way the outputs from one template are passed as parameter values for another template. Take a look at the VPC stack, which is the first thing the “master” template deploys. The customer has either entered a custom VPC CIDR block and subnet CIDR blocks or has kept the default values; these parameter values are passed to the VPC submodule. Later on in the template, the “workload” template that we built earlier is deployed. The outputs from the VPC stack—the VPC ID and the subnet IDs—are passed to the “workload” stack.
